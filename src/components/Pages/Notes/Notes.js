...
class About extends Component {
    ...
    render() {
        let topContent = (
            <>
                <Input
                    id={this.state.postNote.elementConfig.id}
                    type={this.state.postNote.elementConfig.type}
                    placeholder={this.state.postNote.elementConfig.placeholder}
                    value={this.state.postNote.fromDB.post}
                    onChange={(event) => {
                        this.changeHandler(event);
                    }}
                />
                <br />
                <Button id="log" color="success" onClick={this.sendPost} disabled={this.isTextNotValid()}>
                    Log
                </Button>
            </>
        );
        ...
        if (this.props.posts.length > 0) {
            postContents = this.props.posts.map((data) => (
                <Post
                    key={data.id}
                    post={data.post}
                    time={data.time}
                    id={data.id}
                    clicked={this.getElement}
                    postDelete={(event) => {
                        this.getElement(event, data.id);
                    }}
                />
            ));
            ...
            modal = (
                <Modal
                    ...
                    toggle={this.changeModalStateHandler}
                ...
                >
                    ...
                    <ModalBody>
                        <Input
                            type={this.state.selectedPost.elementConfig.type}
                            id={this.state.selectedPost.elementConfig.id}
                            value={this.state.selectedPost.fromDB.post}
                            onChange={(event) => {
                                this.changeHandler(event);
                            }}
                    ...
                    <ModalFooter>
                        <Button
                            id="edit"
                            color={this.state.edit ? 'success' : 'info'}
                            onClick={this.state.edit ? this.updatePost : this.switchToEdit}
                            disabled={this.isUpdatedTextNotValid(this.state.edit)}
                        >
                            {this.state.edit ? 'Submit' : 'Edit'}
                        </Button>
                        <Button color="dark" onClick={this.changeModalStateHandler}>
                            Cancel
                        </Button>
                    </ModalFooter>
                </Modal>
...
const mapDispatchToProps = (dispatch) => {
    return {
        onSendingPost: (post, token, userId) => {
            dispatch(actions.post(post, token, userId));
        },
        onRetrievingPosts: (token, userId) => {
            dispatch(actions.get(token, userId));
        },
        onUpdating: (postUpdate, token, userId) => {
            dispatch(actions.update(postUpdate, token, userId));
        },
        onDeleting: (token, userId, id) => {
            dispatch(actions.deletePost(token, userId, id));
        },
    };
};
